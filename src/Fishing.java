...
public class Fishing {
	...
	public void setupFishing(int fishType) {
		if (c.playerHasItem(getEquipment(fishType))) {
			...
			if (c.playerLevel[c.playerFishing] >= req) {
				if (c.freeSlots() > 0) {
					if (canFishOther(fishType)) {
						c.addItem(otherFishId(fishType), 1);
						c.addSkillXP(otherFishXP(fishType),
								c.playerFishing);
					} else {
						c.addItem(fishType, 1);
						c.addSkillXP(exp * Config.FISHING_EXPERIENCE,
								c.playerFishing);
					}
